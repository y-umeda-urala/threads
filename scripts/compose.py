...
**「この出来事は、福井で人を採りたい会社・発信したい会社に何が起きる？」**
...
発信について
- 自社のSNSが3か月止まっている
- 担当者が退職して、アカウントが誰のものか分からなくなった
- サイトの「お知らせ」の最終更新が2年前
- 投稿が続かない理由を「写真がない」と説明している
- 従業員に任せたら、負担が一人に偏った
- 競合のアカウントが伸びているのを見た
- 取引先に「SNS見ましたよ」と言われて、何もないことに気づいた
- 展示会・新商品・リニューアルのタイミングが来た
- 写真はあるが、文章が書けない
- 発信しているが、何を測ればいいか分からない

### 扱わないこと（2026-09-14 代表指示）

**採用・会社説明会・ミテカラの話題は、このアカウントでは扱わない。**
説明会の日程、採用担当の負担、応募者の理解度、動画での説明、外国人材への説明——
これらを入口にも着地にもしない。上の「発信について」の範囲だけで書く。
"""

DEPTH = {6: "A", 8: "B", 10: "A", 12: "B", 14: "A", 15: "B", 17: "A", 19: "A", 21: "A", 23: "B"}
...
SLOTS = [
    ...
    (21, "福井の話題紹介", "F（紹介型）", "夜、ゆっくりした時間に読まれる想定"),
...
def build_prompt(board: str, neta: str, articles: str, works: str, recent: str, target_date, needed, filled) -> str:
    ...
    sections = [
        ...
        "",
        "### 19:00 の「行けるところまとめ」（2026-09-19 新設・毎日）",
        "",
        "**この枠だけは1件紹介ではなく、3〜5件を並べたまとめです。**",
        "プロフィールに「19時に、いま行けるところをまとめます」と書いてあります。**毎日必ず作ってください。**",
        "",
        "ねらいは、その場で読み捨てられずに**保存される**ことです。単発の告知は今日で用が済みますが、",
        "まとめは「あとで見返す」ために手元に残ります。フォローの一歩手前の行動がここで起きます。",
        "",
        "形：",
        "",
        "  本文（80字以内）… 何がいくつ載っているかを1行で言う。日付と件数を必ず入れる",
        "  THREAD 1件目以降 … 1件につき1つ。地名／名前／いつ／一言（→ 誰にどう効くか）／出典元：URL",
        "",
        "  例（本文）:",
        "    9月19日と20日、福井で行けるところを4つ。鯖江・小浜・越前・坂井です。",
        "",
        "  例（THREAD の1件）:",
        "    鯖江｜めがねフェス2026（9/19-20）",
        "    めがねミュージアム周辺。産地の工場が開く2日間です。",
        "    出典元：https://example.com/...",
        "",
        "決めごと：",
        "",
        "- **今日・明日・今週末に実際に行けるものだけ。** 終わった催しや来月の話は入れない",
        "- 3件に満たない日は、常設の店・施設・展示で埋めてよい（新店・季節の催し・工房など）",
        "- **市町をばらけさせる。** 同じ市から2件以上並べない（材料が無いときは可）",
        "- 1件ずつに**出典元のURL**を付ける。URLを作らない・推測しない",
        "- 商品の話はしない。この枠は深さ A（出来事の共有）で止める",
        "- 末尾に1行、問いを置く。例：「どれか行く予定、ありますか。」",
        "",
        "紹介枠以外（ノウハウ・問いかけ）は必須ではありません。",
        "ただし「福井の会社さんと話していると」のように自然に入るなら、入れてください。",
        "",
