...
DEPTH = {6: "A", 8: "B", 10: "A", 12: "B", 14: "A", 15: "B", 17: "A", 19: "C", 21: "A", 23: "B"}
...
def build_prompt(board: str, neta: str, articles: str, works: str, recent: str, target_date, needed, filled) -> str:
    ...
    sections = [
        ...
        "**日々URALA の記事に限りません（2026-09-15 変更）。** 次の順で材料を探してください。",
        "",
        "  ① 下の「URALAサイトの新着記事」または「ウララコミュニケーションズの制作実績」",
        "     → 選んだ場合は、その記事URLを本文または THREAD の末尾にそのまま書く",
        "       （この枠に限り、下の文体ルールの「リンクは貼らない」を適用しません）",
        "  ② 下の「ネタ帳」にある福井の出来事（ふーぽ・フクブロ・自治体・主催者などから集めたもの）",
        "     → **他社メディアのURLは貼らないこと。** 日々URALA と同じ福井のローカルメディアなので、",
        "       そこへ誘導する形にはしません。**出来事そのもの**（日付・場所・内容）を書きます。",
        "       主催者や店の公式URLがネタ帳にあるときは、それなら貼ってかまいません",
        "",
        "①②のどちらも無いときだけ、無理に作らず柱を「お役立ち・問いかけ」に読み替えてください。",
        "他社メディアの記事本文を写さないこと。書くのは、そこで知った出来事の事実だけです。",
        "",
    ]
    if already:
        sections += [
    ...
    sections += [
        *learning_section(),
        ...
        "- リンクは貼らない（例外は「福井の話題紹介」の枠。日々URALAの記事URL、または主催者・店の公式URLのみ可。他社メディアのURLは貼らない）",
