...
SLOTS = [
    ...
    (19, "先の予定のおすすめ", "G（まとめ型）", "夕方に、この先の予定を決める人に読まれる。4日後以降の催しを2〜3件並べる。プロフィールで毎日19時と約束している枠"),
...
def build_prompt(board: str, neta: str, articles: str, works: str, recent: str, target_date, needed, filled) -> str:
    ...
    sections = [
        ...
        "",
        "ただし **同じ日に 2 回は出せません。** 12:00 と 21:00 では違う催しを選ぶこと",
        ...
        "19:00 のおすすめまとめが「これから」を扱うので、6:00 は「今日まで」を受け持ちます。",
        "",
        ...
        "形：",
        "",
        "  本文（80字以内）… 今日で終わるものが何件あるかを1行で言う。「今日まで」と必ず入れる",
        "  THREAD 1件目以降 … 1件につき1つ。地名／名前／今日までであること／一言／出典元：URL",
        "",
        "  例（本文）:",
        "    今日で終わる催しが福井に4つ。勝山・敦賀・越前町・坂井です。",
        "",
        "  例（THREAD の1件）:",
    ...
    sections += [
        *learning_section(),
        ...
        "",
        "**12:00 と 21:00（先の予定の枠）も、1 日 1 本の決まりは守ります。**",
        "この 2 枠は「日をまたいで何度でも出してよい」という意味で制限の外であって、",
        "**同じ日に同じ催しを 2 回出してよい、という意味ではありません。**",
        "12:00 と 21:00 で違う催しを選んでください。",
        "",
        "**19:00 のまとめ枠も同じです。** まとめに入れる催しは、その日の他の枠で",
        "使っていないものから選んでください。",
        "",
    ...
    for hour, *_ in needed:
        ...
        if 重なり:
            ...
            fail(
                ...
                f"（出典 {sorted(重なり)[0]}）。"
                "同じ出来事は 1 日 1 本までです。別の催しを選んでください。"
            )
